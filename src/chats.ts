...
async function mount(): Promise<void> {
  ...
  let sessionId = requestedSession || newSessionId();
  const sessionChip = root.querySelector<HTMLElement>('.session-chip')!;
  const updateSessionUrl = (id: string) => { sessionId = id; sessionChip.textContent = `Session ${id.slice(0, 8)}`; window.history.replaceState(null, '', `/chats?session=${encodeURIComponent(id)}`); };
  ...
  await refreshChats();
  if (requestedSession) {
    const existing = await getChat(requestedSession);
    if (existing) {
      activeId = existing.id;
      updateSessionUrl(existing.id);
      turns = existing.messages.filter((turn) => typeof turn.content === 'string');
  ...
  root.querySelector<HTMLButtonElement>('.new-chat-btn')!.addEventListener('click', () => { activeId = null; turns = []; input.value = ''; root.querySelector('h2')!.textContent = 'New Chat'; updateSessionUrl(newSessionId()); renderMessages(); });
